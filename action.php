...
switch ($_['action']){
	...
	case 'synchronize':
		...
		if($synchronisationType=='graduate'){
			...
			if(isset($_['format'])) echo 'Synchronisation graduée...'."\n";

		}else{

			$feeds = $feedManager->populate('name');

			if(isset($_['format'])) echo 'Synchronisation complete...'."\n";

		}	

			if(isset($_['format'])) echo count($feeds).' Flux a synchroniser...'."\n";
			

		foreach ($feeds as $feed) {

			if(isset($_['format'])) echo date('H:i:s').' - Flux '.$feed->getName().' ('.$feed->getUrl().') parsage des flux...'."\n";

			$feed->parse();

			if(isset($_['format'])) echo date('H:i:s').' - Flux '.$feed->getName().' ('.$feed->getUrl().') supression des vieux evenements...'."\n";

			if($maxEvents!=0) $feed->removeOldEvents($maxEvents);

			if(isset($_['format'])) echo date('H:i:s').' - Flux '.$feed->getName().' ('.$feed->getUrl().') termin&eacute;'."\n";

			

		}

			if(isset($_['format'])) echo date('H:i:s').' - Synchronisation terminée ( '.number_format(microtime(true)-$start,3).' secondes )'."\n";
